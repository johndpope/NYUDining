...
class RFDiningLocation: NSObject {
    ...
    private func normalizedTime() -> Date {
        let now = Date()
    ...
    private func isTimeInRange(_ timeRange: String) -> Bool {
        ...
        if let openTime = openTime, var closeTime = closeTime {
            ...
            let diffComparison = calendar.compare(openTime, to: closeTime, toUnitGranularity: .hour)
            if diffComparison == .orderedDescending {
                closeTime = calendar.date(byAdding: .day, value: 1, to: closeTime, options: [])!
            }
            
            let openComparison = calendar.compare(now, to: openTime, toUnitGranularity: .minute)
            let closeComparison = calendar.compare(now, to: closeTime, toUnitGranularity: .minute)
